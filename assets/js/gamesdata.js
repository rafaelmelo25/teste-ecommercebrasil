        ...
        const gamesData = [
                      ...
                      <div class="col-12">
                          <div class="card">
                              ...
                              <div class="card-body">
                                  ...
                                  <ul class="d-flex justify-content-between align-items-bottom">
                                      <li>
                                          <button class="btn details-button">Detalhe</button>
                                      </li>
                                      <li>
                                          <p class="game-price top" data-original-price="${game.salePrice}"></p>
                                          <p class="game-price bottom" data-original-price="${game.normalPrice}"></p>
                                      </li>
                                      <li>
                                          <button class="btn discount-button" disabled>${(game.savings * 1).toFixed(2)}%</button>
                                      </li>
                                  </ul>
                              </div>
                          </div>
          ...
          function showGameCards(start, end) {
              // Limpa os cartões existentes
              gameCardsContainer.innerHTML = "";
          
              // Cria os novos cartões para exibição
          ...
          function loadGames() {
              showGameCards(0, cardsToShow);
          }
          
          // Event listener para o campo de pesquisa
          function sortGamesBy(option) {
              let filteredGames = gamesData;
          
              if (option === 'discount') {
                  filteredGames = filteredGames.sort((a, b) => parseFloat(b.savings) - parseFloat(a.savings));
              } else if (option === 'highest-price') {
                  filteredGames = filteredGames.sort((a, b) => parseFloat(b.normalPrice) - parseFloat(a.normalPrice));
              } else if (option === 'lowest-price') {
                  filteredGames = filteredGames.sort((a, b) => parseFloat(a.normalPrice) - parseFloat(b.normalPrice));
              } else if (option === 'title') {
                  filteredGames = filteredGames.sort((a, b) => a.title.localeCompare(b.title));
              }
          
              gameCardsContainer.innerHTML = ''; // Limpa os cartões existentes
              showGameCards(0, cardsToShow, filteredGames); // Mostra os cartões classificados
          }
          
          // evento para o seletor "Ordenar por"
          const orderBySelect = document.getElementById('sortSelect');
          orderBySelect.addEventListener('change', () => {
              const selectedOption = orderBySelect.value;
              sortGamesBy(selectedOption);
          });
          
          // Função para carregar e exibir os jogos iniciais          
          loadGames();
          
        //   function loadMore() {
        //       const currentCards = document.querySelectorAll(".col-md-4");
        //       const start = currentCards.length;
        //       const end = start + cardsToAdd;
          
        //       if (start >= gamesData.length) {
        //           loadMoreButton.style.display = "none"; // Oculta o botão quando todos os jogos foram carregados
        //           return;
        //       }
          
        //       showGameCards(start, end);
        //   }
        ...
        let currentIndex = 0;
        function loadMore() {
            const totalCards = gamesData.length;
            const maxIndex = currentIndex + cardsToAdd;
        
            for (let i = currentIndex; i < maxIndex && i < totalCards; i++) {
                createGameCard(gamesData[i]);
            }
        ...
        loadMoreButton.addEventListener("click", loadMore);
          ...
          searchInput.addEventListener("input", () => {
              const searchTerm = searchInput.value.trim().toLowerCase(); // Obtenha o termo de pesquisa e normalize
          
              if (searchTerm === "") {
                  // Se o campo de pesquisa estiver vazio, carregue todos os jogos novamente
                  loadGames();
              } else {
                  const filteredGames = filterGames(searchTerm);
          
                  // Exiba apenas os jogos correspondentes à pesquisa
                  showGameCards(0, filteredGames.length);
          
                  if (filteredGames.length === 0) {
                      // Se não houver jogos correspondentes à pesquisa, exiba uma mensagem ou ação apropriada
                      gameCardsContainer.innerHTML = "<p>Nenhum jogo encontrado.</p>";
                  }
              }
          });
